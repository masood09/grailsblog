...
<html>
    <head>
        ...
        <title>Grailsblog - ${categoryInstance.name}</title>
    </head>
    <body>
        <g:each in="${categoryInstance.getPosts()}" status="i" var="postInstance">
            ...
            <g:if test="${categoryInstance.getPosts().size() != (i+1)}">
                <hr>
                <br>
            </g:if>
        </g:each>

        <g:if test="${categoryInstance.getPosts() == 0}">
            <h2>No posts found for the category ${ categoryInstance.name }</h2>
        </g:if>
    </body>
</html>
